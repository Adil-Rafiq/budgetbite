    ...
    <section className={cardClass}>
      <div className="flex items-center justify-between gap-3 border-b border-sage/70 px-5 py-3">
        <h2 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate">
          <Icon aria-hidden className="h-3.5 w-3.5 text-dark-green" />
          {title}
        </h2>
        <button
          type="button"
          onClick={() => actions.goToStep(editStep)}
          className={`-mr-2 min-h-11 rounded-lg px-2 text-xs font-semibold text-dark-green underline underline-offset-2 transition-colors hover:text-charcoal ${FOCUS_RING}`}
        >
          {editLabel}
        </button>
      </div>
