    ...
    return (
      ...
      >
        ...
        <div className="flex flex-wrap justify-center gap-2">
          ...
          <div className={cn(shimmerCls, 'h-3 w-16 bg-sand')} />
          <div className={cn(shimmerCls, 'h-3 w-12 bg-sand')} />
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-1 items-end justify-between gap-3 px-2">
          {BAR_HEIGHTS_A.map((h, i) => (
            <div key={i} className="flex h-full flex-1 items-end justify-center gap-1">
              <div className="w-3 animate-pulse rounded-sm bg-sand" style={{ height: h }} />
              <div
                className="w-3 animate-pulse rounded-sm bg-sand opacity-60"
    ...
    <div className={cn('relative flex h-full w-full flex-col gap-2', className)}>
      <div className="relative flex flex-1 items-end justify-between gap-1 px-1">
        {[0, 1, 2, 3].map((i) => (
          <div
            key={`grid-${i}`}
            className="absolute left-0 right-0 h-px bg-sand opacity-50"
            style={{ bottom: `${(i + 1) * 20}%` }}
          />
        ))}
        {LINE_HEIGHTS.map((h, i) => (
          <div key={i} className="relative flex h-full flex-1 items-end justify-center">
            {variant === 'area' && (
              <div
                className="absolute bottom-0 left-0 right-0 animate-pulse bg-sand opacity-60"
                style={{ height: h }}
              />
            )}
            <div
              className="size-2 animate-pulse rounded-full bg-sand"
