    ...
    <div className="flex flex-col gap-3">
          ...
          return (
            <button
              ...
              className={`inline-flex min-h-11 items-center gap-2 rounded-full border px-4 py-2 text-[13px] capitalize transition-colors ${FOCUS_RING} ${
                checked
                  ? 'border-teal-ink bg-teal/10 font-semibold text-charcoal'
                  : 'border-sand bg-surface font-normal text-slate hover:border-teal-ink/50'
              }`}
            >
              <span
                aria-hidden
                className={`flex h-4 w-4 items-center justify-center rounded-full border transition-colors ${
                  checked ? 'border-teal-ink bg-teal-deep text-white' : 'border-sand bg-surface'
                }`}
              >
      ...
      <div className="flex gap-2">
        <input
          ...
          className={`min-h-11 w-full rounded-xl border border-sand bg-surface px-3.5 py-2 text-[13px] text-charcoal transition-colors placeholder:text-slate/50 focus:border-teal-ink ${FOCUS_RING}`}
        />
        <button
          type="button"
          onClick={handleAdd}
          disabled={draft.trim().length === 0}
          className={`min-h-11 rounded-xl border border-sand bg-canvas px-4 py-2 text-[13px] font-medium text-charcoal transition-colors hover:border-teal-ink disabled:opacity-40 disabled:hover:border-sand ${FOCUS_RING}`}
        >
          Add
        </button>
      </div>
