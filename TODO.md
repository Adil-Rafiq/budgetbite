...
- [x] Weekly email digest via Resend (spent X of Y, week summary): a "spent X of your Y budget" progress email with a budget bar, an on-track/over-pace badge, a trailing-7-day activity summary (meals logged + spent this week), and a dashboard CTA. Sent per eligible active plan (owner has a verified email, today inside the plan's date window) via `POST /api/cron/digests/weekly` — a dedicated `/api/cron/*` surface gated by its own least-privilege `CRON_SECRET` (`X-Cron-Secret` header; 503 when unset), separate from `ADMIN_API_KEY`, since there's no in-process worker tier. Idempotent within a week: a `budget_plan.last_weekly_digest_sent_at` marker (migration `0018_skinny_colossus.sql`) stamped only after a confirmed send + a ~6-day cooldown means a cron misfire or manual re-run never double-emails. Response is `{ total, sent, skipped, failed }`; each send is isolated so one failure never aborts the batch. New `budgetPlanRepository.listActiveForDigest()` / `markWeeklyDigestSent()` and `orderRepository.getWeeklyStatsByPlans()` (grouped 7-day count/spend); pure `isOnBudgetPace` / `daysRemaining` helpers in `plan-math.ts` (unit-tested); `WeeklyDigestResult` contract in `@repo/shared`. Reuses `WEB_URL` for the CTA link and the existing Resend `EMAIL_FROM` sender.
